...
export default async function decorate(block) {
  ...
  if( userId == null || userId == undefined || accessToken == null || accessToken == undefined){
    ...
    await fetch(DYNAMIC_365_DOMAIN + "insided_get_user_link?auth0_id=" + userId, {
      ...
      let userLink = (data.link == null || data.link == undefined) ? 'https://community.zemax.com/ssoproxy/setUsernameForm':data.link;
      anchor.setAttribute('href', userLink);
      anchor.setAttribute('target', '_blank');
      anchor.classList.add('button', 'primary');
      // Use placeholder
      var link = document.createTextNode("Forum Profile & Activity");
